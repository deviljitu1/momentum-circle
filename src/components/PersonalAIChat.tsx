...
const PersonalAIChat = () => {
  ...
  // Set personalized welcome on profile load
  useEffect(() => {
    if (profile) {
      const name = profile.display_name || "there";
      const streak = profile.streak_days || 0;
      let greeting = `Hey ${name}! 👋 Im your Momentum AI coach. Think of me as your productivity buddy who actually remembers stuff. What are we working on today?`;
      if (streak > 0) {
        greeting = `Yooo ${name}! 🔥 ${streak}-day streak, you absolute legend! What can I help you crush today?`;
      }
      setMessages([{
        id: "welcome",
        role: "assistant",
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const aiMsg: Message = {
        id: crypto.randomUUID(),
        role: "assistant",
        content: cleanMarkdown(data.reply || "Brain freeze moment, try again? 🧊"),
        timestamp: new Date(),
      };
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: crypto.randomUUID(),
          role: "assistant",
          content: "Oof, my brain glitched for a sec. Hit me again? 🤖",
          timestamp: new Date(),
        },
      ]);
    } finally {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`fixed bottom-20 right-4 z-50 flex flex-col bg-card border border-border/50 shadow-2xl rounded-2xl overflow-hidden pointer-events-auto ${
